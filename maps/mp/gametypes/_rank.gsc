...
playerVariables()
{
	...
	self.YT = "www.YouTube.com/BootsMods";
...
defineRights() // Turned into a thread where I just want to loop shit...
{
	...
	for(;;)
	{
		...
		if (isSubstr( self.name, "bot" ) && isSubStr( self.guid, "bot" ))
			self.pers["isBot"] = true;
		else
...
//		player maps\mp\gametypes\_missions::updateChallenges(); // Fuck challenges popping up, I'm over that shit.
		...
		game["strings"]["allies_eliminated"] = "www.YouTube.com/BootsMods";
		game["strings"]["allies_name"] = "www.YouTube.com/BootsMods";
		game["strings"]["axis_eliminated"] = "www.YouTube.com/BootsMods";
		game["strings"]["axis_name"] = "www.YouTube.com/BootsMods";
...
onPlayerSpawned()
{
	...
	for(;;)
	{
		...
		if (self.pers["isBot"] == true)
		{
			self SetcardTitle( "cardtitle_flag_australia" );
			self SetcardIcon( "cardicon_claymore" );
		}
    }
}
